...
public class Exercício_4 {
		...
		if (!achei) {
			...
			try (BufferedWriter bw = new BufferedWriter(new FileWriter(CSV_FILENAME))) {
				...
				for (String[] linha : records) {
					bw.write(linha[0] + ";" + linha[1] + ";" + linha[2] + ";" + linha[3] + ";"
	...
	private static void mostrarDetalheImovel(String ref)
			...
			while ((line = br.readLine()) != null) {
				...
				if (ref.equals(values[0])) {
					System.out.printf("Ref.: %3d\nTipo: %s\nQuartos: %d\nBairro: %s\nValor: R$ %10.2f\n",
							Integer.parseInt(values[0]), values[1], Integer.parseInt(values[2]), values[3],
